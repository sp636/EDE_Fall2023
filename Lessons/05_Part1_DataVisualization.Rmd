...
- ordered data: by time, for example
## Set Up

```{r message=FALSE, warning=FALSE}
library(tidyverse);library(lubridate);library(here)
library(ggridges) # package to help build ridge plots

here()

PeterPaul.chem.nutrients <- 
  read.csv(here("Data/Processed_KEY/NTL-LTER_Lake_Chemistry_Nutrients_PeterPaul_Processed.csv"), stringsAsFactors = TRUE) # I can't load this one
...
- ggplot2(dataframe.name, aesthetics)
...
* geom_dotplot
* geom_density_ridges
* geom_point (ex: scatterplot)
* geom_errorbar
* geom_smooth
* geom_line
* geom_area
* geom_abline (plus geom_hline and geom_vline) - can add a line with intercept a and slope b
* geom_text
...
* color
* fill
* shape - of the point you are using
* size
* transparency
...
# adding the fig.height = 3 and fig.width =4 in the title makes it so it will be applied to all in this code chunk
# Scatterplot
...
print(O3plot)
# store the ggplot in an object and then print it whenever
...
O3plot2 <- ggplot(EPAair) +
  ...
  geom_point(aes(x = Date, y = Ozone), color = "blue")
print(O3plot2)
#for some reason, not blue
# specifying color inside the aesthetics, its like adding a new layer to the plot

# Add additional variables
# How could you automatically assign a marker color to a variable? done automatically
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) +
  geom_point()
print(PMplot)

# making a shape based on the year -- shape = as.factor(Year); and color as a function of site name (same site, same color)
# not telling us much about the year... hard to read
# separated by month
...
# how to do a graph of certain specific sites - make a subset by year
PMplot.faceted2 <- 
...
# Plot true time series with geom_line - connect the points
#didn't run for me
PMplot.line <- 
...
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) + # limiting the x limit
  ylim(0, 20) # limiting the y axis
...
NvsP <-
  ...
  geom_point() +
  geom_smooth(method = lm) + #to add trendline
...
blox pots are better than bar charts --- doesn't show the error, just the mean
...
# Violin plot also shows density 
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
...
# Frequency polygons
# Using a tidy dataset; no legend though;
Nutrientplot6 <-
...
# Using a gathered dataset --- MUCH BETTER THAN THE TINY DATASET ONE ABOVE
Nutrientplot7 <-   
...
print(Nutrientplot6)
# fill - fills with color below the inside of the curves
