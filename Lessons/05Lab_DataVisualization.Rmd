...
ggplot(data=data.frame(x=0,y=0))+
  ...
  geom_text(
    x=0, #specifying where we want the text; and the following line too
    y=0.025,
    label=expression(paste("Mean is ", mu,",", R^{2},'=0.6'))
    ) # paste function just pastes the characters together; asking for the greek letter mu. if you want a subscript use "_"
...
ggplot(PeterPaul.chem.nutrients,
       aes(x=month))+
  geom_bar()
# hard to read this plot; month column is NOT a factor; its an integer
factor(PeterPaul.chem.nutrients$month, levels = 1:12, labels=month.abb)
#month.abb does an abbreviated format
#not changing the original dataset, just doing this nested function
...
ggplot(PeterPaul.chem.nutrients,
  ...
  # which scale x to use: depends on the class of teh column; for this one - use discrete because FACTORS ARE JUST CATEGORIES
...
# 1. 
# Plot surface temperatures (x axis) by day of  year (y axis). 
...
ggplot(EPAair, aes(
  ...
  geom_point(alpha=0.5)+ #alpha changes the transparency of the plots
