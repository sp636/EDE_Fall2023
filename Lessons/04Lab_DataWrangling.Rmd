...
glimpse(NTL.phys.data$sampledate) # like the structure function
...
# dplyr filtering, filtering the data such that we are getting all the rows where depth = 0
NTL.phys.data.surface <- filter(NTL.phys.data, depth == 0)

# Choose multiple conditions to filter
summary(NTL.phys.data$lakename) # reports all the unique values - can use the unique() or levels()
...
# what code do you need to use, based on the class of the variable? 1898 rows
...
# can look at only the month of the date column or can make a new column for month
...
# filter for depth==0). Make sure you eliminate NAs before computing the means - dont have to.
