...
## Objectives
1. Discuss the purpose and application of time series analysis for environmental data - consistent
...
library(trend)
#install.packages("zoo") replaces missing operations
library(zoo)
#install.packages("Kendall")
library(Kendall)
#install.packages("tseries") not going to use it today, but has some useful things
library(tseries)
...
components:
- trend
- seasonal component (cyclical, usually related to calendar events)
- cycle: similar to seasonal, no relation to calendar. can be something that happens every 50 years or something; a business cycle; a longer duration than a year

decomposing gives you three parts: trend, seasonal, and random (unexplanable variation in data)

stationarity test: if mean and stdev are constant through time
if data is not stationary -

DO NOT REMOVE NA's

monotonic trends: always increasing or decreasing; tests to statistically prove if there is a trend

why fit a timeseries model to data? we have a variable and we don't know what it is giong to do; may be important for decisions for today. going to try to replicate into the future; looks at historical data
...
```{r}
#split up the dataset because there is a major gap
...
2. A **trend** component, which quantifies the upward or downward progression over time. The trend component of a time series does not have to be monotonic. (may be stochastic too)
...
```{r}
#defining the time series data  - specify when it started; year, then month and frequency = 365 if daily data
# to see it, type in the console: head(xxxts, 10) to show the first 10
...
```{r}
?stl # univariate time series is the first argument, run the decomposition for each variable separately
# if you do s.window as "periodic" it will be what you specified. or s.window = 365 
...
# adding in some info so we can plot it
...
ggplot(EnoDischarge.late_Components) +
  ...
  geom_hline(yintercept = 0, lty = 2) + # adds a horizontal line at y=0
  ylab(expression("Discharge (ft"^3*"/s)"))

# Visualize how the seasonal cycle maps onto the data, trend line dips below 0
ggplot(EnoDischarge.late_Components) +
  geom_line(aes(y = Observed, x = Date),  size = 0.25) +
...
```{r}
head(wind_data)
summary(wind_data$AWND) # shows the number of NA's
...
# zoo package and na.approx function
...
f_month <- month(first(wind_data_clean$DATE)) # generate first month of first observation of this column
f_year <- year(first(wind_data_clean$DATE)) # f year is first observation's year
wind_data_ts <- ts(wind_data_clean$AWND.clean,
                   start=c(f_year,f_month),
                   frequency=12) # 12 because monthly data
