...
> Add notes here: for synchron session

```{r setup workspace, message = FALSE}
# Step 1 install the packages needed then make them comments so itll knit later
getwd()
#install.packages(tidyverse)
library(tidyverse)
#install.packages(lubridate)
library(lubridate)
# step 2: import the data set of chemistry physics data in RELATIVE PATH, make sure it shows up in the environment
NTL.phys.data <- read.csv("./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv", stringsAsFactors = TRUE)
# add "stringasfactors=TRUE" !!

colnames(NTL.phys.data) # gives us the column names, different depths at same time
head(NTL.phys.data) # first six columns
summary(NTL.phys.data)
str(NTL.phys.data)
dim(NTL.phys.data)

class(NTL.phys.data$sampledate)
# Format sampledate as date through as.Date function bc its a character currently
#after, as for the class of the sample date again and it should be a date
# can do head() to make sure its formatted well
...
write a simple code that gives you the correct outputs
...
```{r, results = "hide"}
vignette("dplyr")
# will give you an intro to dplyr
# provides a function for each basic verb
# three categories: rows - use filter() - chooses based on column values
# slice() : chooses rows based on location
# arrange() changes the order of rows
# 2nd category: COLUMNS (select, rename, mutate, relocate)
# 3rd category: GROUP ROWS - collapses a group into a single row
```
...
`==` # compares your value to a specific value
`!=`# checks if the variables are different
`<` values that are less than a specific number
`<=`less than or equal to a specific number
`>`greater than a specific number
`>=`greater than or equal to a specific number
`&` multiple conditions if you need both to be true
`|` multiple conditions if you need just one (OR)

```{r filtering data}

# can only compare character with character and numeric with numeric
class(NTL.phys.data$lakeid)
class(NTL.phys.data$depth) # is a dataframe, and works like a matrix so can use the matrix notation


# matrix filtering - creating a subset of data, assign it to the original data set and filter only the rows that have 0 for column depth
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,]

# dplyr filtering -- now the filter function from dplyr. column depth == 0
NTL.phys.data.surface2 <- filter(NTL.phys.data, depth == 0) # is this the same? basically, more flexible
NTL.phys.data.surface3 <- filter(NTL.phys.data, depth < 0.25) 
# depth that is less than 0.25, near the surface
...
summary(NTL.phys.data$lakename) # number of occurences of each name
# below are three different ways to select peter or paul lake. FILTER ONLY CHOOSES ROWS

# filter(dataname, columnname == specific name | lakename == specific name)
NTL.phys.data.PeterPaul1 <- filter(NTL.phys.data, lakename == "Paul Lake" | lakename == "Peter Lake")

# the one below is how to exclude a lakename
...
# this one is simpler: lakename %in% c("specific lake name", "specific lake name")
...
# daynum is what number of the day of the year it is (ex: jan 1 is 1, dec 31 is 365. first day of june is 152 and last day of october is 304)
summary(NTL.phys.data$daynum)
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305)
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) # can use ,
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304)
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) #include all acceptable
...
```{r arranging data}
# arrange by a certain column (depth)
...
# new data frame, just want to know about the lake name, and want the same columns  (sample date, depth, temperature_C) since sampledate:temperature_C are adjacent, can just do :
# creates a tidy data set and gets rid of all the other random things!
```
### Mutate
...
# want to include a new column with temp in F.
# adding a new column to an existing dataset
# dataset <- mutate(datasetyoumade, name of new column = (temperature_C*9/5)+32)
```
...
```{r using lubridate}
# add a month column to the dataset, an existing dataset
# month from (sampledate) to make a new column from the date
...
# want to put the month column between lakeid:daynum and sampledate:comments
NTL.phys.data.PeterPaul1 <- select(NTL.phys.data.PeterPaul1, lakeid:daynum, month, sampledate:comments)

# find out the start and end dates of the dataset; want to select an interval or check the first and last observation of data
...
# use the second approach! good for any size dataset
```
...
```{r using pipes}
# creating a new dataframe ....processed, start from original name, THEN filter by lake name, THEN select sampledate:temp_C, THEN mutate to make temp_F (don't need to specify dataset because all done in a pipe)
# soooo much cleaner to do all the things we just went through
...
# should write the csv, do the relative path
# end of the wrangling of this data set
```
...
It's better to keep an RMD file with all the wrangling, then have a SEPARATE file for the ANALYSIS and VISUALIZATION. cleaner.
